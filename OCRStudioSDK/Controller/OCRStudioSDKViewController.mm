...
- (UIButton *) torchButton {
  if (!_torchButton) {
    ...
    if (@available(iOS 13.0, *)) {
      UIImage *btnImage = (self.torchOnByDefault) ? [ UIImage systemImageNamed:@"bolt.fill"] : [UIImage systemImageNamed:@"bolt"];
...
- (void) torchButtonTapped {
  if ([self.camera isTorchOn]) {
    ...
    if (@available(iOS 13.0, *)) {
      UIImage *btnImage = [UIImage systemImageNamed:@"bolt.fill"];
      [_torchButton setImage:btnImage forState:UIControlStateNormal];
    } else {
      // Fallback on earlier versions
    }
    ...
    [self.camera turnTorchOnWithLevel:1.0];
    if (@available(iOS 13.0, *)) {
      UIImage *btnImage = [UIImage systemImageNamed:@"bolt"];
      [_torchButton setImage:btnImage forState:UIControlStateNormal];
    } else {
      // Fallback on earlier versions
    }
